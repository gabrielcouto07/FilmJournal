...
/// `/account/*` — exclusão de conta e troca de e-mail/senha.
public final class AccountService {
    ...
    public func changeEmail(to email: String, currentPassword: String) async throws -> String? {
        let response: MessageResponse = try await client.request(.post, "/account/email", body: ChangeEmailRequest(email: email, currentPassword: currentPassword))
        return response.message
    }
    ...
    public func requestPasswordChange(currentPassword: String, newPassword: String) async throws -> ChangePasswordResponse {
        try await client.request(.post, "/account/password", body: ChangePasswordRequest(currentPassword: currentPassword, newPassword: newPassword))
    }
    ...
    public func confirmPasswordChange(code: String) async throws -> String? {
        let response: MessageResponse = try await client.request(.post, "/account/password/confirm", body: ConfirmPasswordChangeRequest(code: code))
        return response.message
    }
}

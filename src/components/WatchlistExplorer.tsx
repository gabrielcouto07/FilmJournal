...
export default function WatchlistExplorer({ initialMovies }: { initialMovies: WatchlistMovie[] }) {
  ...
  return <div>
    ...
    {!visible.length ? <div className="empty-state mt-6"><p className="text-lg font-bold text-white">Sua lista para assistir está vazia.</p><p className="mt-2 text-sm text-slate-500">Altere os filtros ou descubra um filme que valha a pena salvar.</p><Link href="/search" className="accent-button mt-5">Descobrir filmes</Link></div> : view === "grid" ? <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7">{visible.map((movie) => {
      const preferredPoster = getPosterUrl(movie.preferredPosterPath); const defaultPoster = getPosterUrl(movie.posterPath);
      return <article key={movie.id} className="poster-card group overflow-hidden rounded-[1.15rem] border border-white/[0.09] bg-[#141414]"><Link href={`/film/${movie.id}`} className="block"><div className="relative aspect-[2/3] bg-white/[0.04]"><ArtworkImage src={preferredPoster} fallbackSrc={defaultPoster} alt={`Pôster de ${movie.title}`} title={movie.title} className="h-full w-full" sizes="(max-width: 640px) 45vw, 190px" /><div className="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-black p-3 pt-12"><h2 className="truncate text-sm font-black text-white">{movie.title}</h2><p className="mt-1 text-[10px] font-bold text-slate-400">{movie.year ?? "—"}{movie.runtime ? ` · ${movie.runtime}m` : ""}</p></div></div></Link><div className="flex gap-1.5 border-t border-white/[0.06] p-2"><Link href={`/film/${movie.id}?log=1`} className="quiet-button flex-1 !px-3 !py-2 text-xs">Registrar</Link><button type="button" onClick={() => remove(movie)} className="icon-button h-9 w-9" aria-label={`Remover ${movie.title} da lista para assistir`}>×</button></div></article>;
    })}</div> : <div className="mt-6 space-y-2">{visible.map((movie) => { const preferredPoster = getPosterUrl(movie.preferredPosterPath); const defaultPoster = getPosterUrl(movie.posterPath); return <article key={movie.id} className="surface flex gap-4 rounded-2xl p-3"><Link href={`/film/${movie.id}`} className="h-24 w-16 shrink-0 overflow-hidden rounded-lg bg-white/[0.04]"><ArtworkImage src={preferredPoster} fallbackSrc={defaultPoster} alt={`Pôster de ${movie.title}`} title={movie.title} className="h-full w-full" sizes="64px" /></Link><div className="min-w-0 flex-1 py-1"><div className="flex flex-wrap items-start justify-between gap-3"><div><Link href={`/film/${movie.id}`} className="font-black text-white hover:text-amber-200">{movie.title}</Link><p className="mt-1 text-xs text-slate-500">{[movie.year, movie.genres, movie.runtime ? `${movie.runtime} min` : null].filter(Boolean).join(" · ")}</p></div><div className="flex gap-2"><Link href={`/film/${movie.id}?log=1`} className="quiet-button !px-3 !py-2 text-xs">Registrar</Link><button type="button" onClick={() => remove(movie)} className="quiet-button !px-3 !py-2 text-xs">Remover</button></div></div><p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-500">{movie.overview || "Sinopse indisponível."}</p></div></article>; })}</div>}
  </div>;
}

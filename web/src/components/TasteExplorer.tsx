  ...
  async function act(movie: TasteRecommendation, action: RecommendationAction) {
    ...
    try {
      ...
      if (action === "open" || action === "log") {
        router.push(`/film/${existing.id}${action === "log" ? "?log=1" : ""}`);
      } else {

...
type PoolMovie = {
  ...
  genreIds: number[];
  /** Motivo da indicação quando ela vem dos pontos cegos. */
  rationale?: string;
  gapLabel?: string;
};
...
// Sorteio sem viés usando números aleatórios seguros.
...
export default function RouletteBoard() {
  ...
  // Alguns filtros só funcionam quando a fonte oferece esses dados.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        /* se falhar, os filtros de gênero ficam vazios */
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        /* se falhar, mantém os filtros padrão */
      }
    })();
    return () => {
      cancelled = true;
  ...
  // Autocomplete com pequeno atraso para reduzir chamadas.
  useEffect(() => {
    ...
    const handle = window.setTimeout(async () => {
      try {
        ...
        /* falha não bloqueia os demais filtros */
      }
    }, 400);
  ...
  // Posiciona o primeiro pôster no marcador antes de girar.
  useEffect(() => {
  ...
  const fetchWinnerDetail = useCallback(async (movieId: number) => {
    ...
    try {
      ...
      /* o resultado já mostra os dados principais */
    } finally {
  ...
  const spin = useCallback(async () => {
    ...
    // Reposiciona numa repetição idêntica para criar espaço sem salto visual.
  ...
  return (
    <div className="space-y-10">
      <header>
        ...
        <h2 className="display-title mt-2 text-4xl sm:text-5xl">Roleta de Filmes.</h2>
              ...
              <div className="relative mx-auto" style={{ height: STAGE_H }}>
                ...
                <div
                  ...
                  style={{
                    ...
                    boxShadow: flash
                      ? "0 0 0 3px rgb(var(--accent-300) / .5), 0 0 60px 12px rgb(var(--accent-300) / .55)"
                      : "0 0 0 2px rgb(var(--accent-300) / .25), 0 0 28px 4px rgb(var(--accent-300) / .28)",
                  ...
                  return (
                    <motion.button
                      ...
                      className={`poster-card group relative block overflow-hidden rounded-xl border text-left transition-shadow disabled:cursor-wait ${
                        isWinner
                          ? "z-10 border-amber-300 shadow-[0_0_28px_rgb(var(--accent-300)/0.5)]"
                          : "border-white/[0.08] hover:border-amber-300/40 hover:shadow-[0_0_22px_rgb(var(--accent-300)/0.22)]"
                      }`}
                    >
      ...
      {/* Revelação montada no body para ficar acima do cabeçalho */}
